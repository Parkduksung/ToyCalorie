...
@HiltViewModel
class Food3ViewModel @Inject constructor(
    app: Application,
    ...
    fun getGroceryList(list: ArrayList<String>) {
        ioScope {
            when (val result = calorieRepository.getLocalCalorieList("반찬")) {
                ...
                is Result.Success -> {
                    ...
                    viewStateChanged(Food3ViewState.GetFoodList(toFoodList))
                }
...
sealed class Food3ViewState : ViewState {
    data class GetFoodList(val list: List<CalorieItem>) : Food3ViewState()
}